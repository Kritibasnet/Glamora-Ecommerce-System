...
function EsewaSuccess() {
    ...
    // Main effect to handle payment verification once context and cart are ready
    useEffect(() => {
        if (!contextValue || processed) return;

        const { cart, checkout, cartTotal } = contextValue;

        // Wait for cart to be available (with a timeout to prevent infinite waiting)
        if (cart.length === 0) {
            return; // Cart not ready yet, will retry on next render
        }
        ...
        const handleProcess = async () => {
            ...
            try {
                ...
                if (response.ok && resultData.success) {
                    ...
                    const currentTotal = pendingData ? pendingData.finalTotal : cartTotal;
        ...
        handleProcess();
    }, [contextValue, processed, location, history]);

    // Timeout effect to prevent infinite waiting for cart to load
    useEffect(() => {
        const timeoutId = setTimeout(() => {
            if (!processed && contextValue && contextValue.cart.length === 0) {
                console.warn('Payment verification timeout: Cart did not load after 10 seconds');
                setStatus('Error');
                setMessage('Cart loading timed out. Please check your order status in your dashboard.');
                setProcessed(true);
            }
        }, 10000); // 10 second timeout

        return () => clearTimeout(timeoutId);
    }, [processed, contextValue]);

    return (
        <ProductConsumer>
            {value => {
                // Update context value for the effect to use
                    ...
                    <div className="container mt-5 text-center">
                        <div className="row">
                            <div className="col-10 mx-auto text-center text-title text-capitalize pt-5">
                                ...
                                {!processed && (!contextValue || contextValue.cart.length === 0) && (
                                    <div className="mt-4">
